		...
		else
		{
			...
			if ($dbInstance->issueCommand($queryString) == true)
			{
				...
				while ($resultSet != FALSE)
				{
					...
					$setting->setId($resultSet->id);
					$setting->setName($resultSet->setting);
					$setting->setValue($resultSet->value);
					...
					$setting->setTimeStamp($resultSet->settingTimeStamp);
					$this->m_settingsArray[$resultSet->setting] = $setting;
					$resultSet = $dbInstance->getResult();
				}
				...
				if ($dbInstance->issueCommand($queryString) == true)
				{
					...
					if ($dbInstance->issueCommand($queryString) == true)
					{
						...
						if ($resultSet != FALSE)
						{
							...
							$setting->setName($resultSet->setting);
							$setting->setValue($resultSet->value);
							$setting->setActive($resultSet->settingActive);
							$setting->setTimeStamp($resultSet->settingTimeStamp);
							$this->m_settingsArray[$resultSet->setting] = $setting;
						}
						$dbInstance->releaseResults();

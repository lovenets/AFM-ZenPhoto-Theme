	...
	*/
	...
	function activate()
	{
		...
		if ($dbInstance->issueCommand($queryString, $extResourceId) == true)
		{
			...
			{
				$instanceValue = (int)$resultSet->instance;
				if ($this->getAllowMultipleInstances() == true)
				{
					$found = false;
					while (($resultSet != FALSE) && ($found == false))
					{
						if ($resultSet->activeFlag == 0)
						{
							$found = true;
							$instanceValue = (int)$resultSet->instance;
						}
	...
	function registerExtension($extPath, $fileName = EXTENSION_FILE_NAME)
	{
 		...
 		if ($dbInstance->issueCommand($queryString, $resourceId) == true)
		{
			...
			while ($resultSet !== FALSE)
			{
				...
				$extensionInstance->setDisplayPosition($resultSet->position);
				$extensionInstance->setDisplayLocation($resultSet->location);
				$extensionInstance->setInstance($resultSet->instance);

...
class PageData
{
    ...
    function PageData()
    {
        ...
        $this->m_displayObjects[RSS_FEED_LINK] = null;
...
//        $renderedPage .= " xmlns:svg=\"http://www.w3.org/2000/svg\"";
	    ...
	    {
	    	$renderedPage .= "<link rel=\"shortcut icon\" href=\"$iconFile\" />";
	    }
        else
        {
        	$renderedPage .= "<!-- no icon $iconFile -->";
        }
...
DISPLAY_DATA;
        ...
        if ($this->m_displayObjects[RSS_FEED_LINK] != null)
        {
        	foreach ($this->m_displayObjects[RSS_FEED_LINK] as $displayObject)
        	{
        		$objectText = $displayObject->getData();
        		$titleData = $displayObject->getRssTitle();
                $displayData = <<<DISPLAY_DATA
    <link rel="alternate" href="${objectText}" type="application/rss+xml" title="${titleData}" />
...
DISPLAY_DATA;
            	...
            	$renderedPage .= $displayData;
                $objectId = $displayObject->getId();
                if ($objectId != null)
                {
                	$renderedPage .= ' id="' . $objectId . '" ';
                }
                $displayData = <<<DISPLAY_DATA
    />
DISPLAY_DATA;
...
class Page extends PageData
{
	 ...
	 * 
	 * @param $headerData - the text data to add to the page - html etc. ok other then <header></header>
	 * 
	...
	function addHeaderData($headerData)
	{
		...
		$headerObject->setData($headerData);
		$this->addDisplayObject($headerObject);
	 ...
	 * addRssFeed
	 *
	 * to add a link for an rss feed for the page
	 *
	 * @param href
	 * @param title
	 *
	 * @return none
	 */
	 function addRssFeed($href, $title)
	 {
	/*
	<link rel="alternate" type="application/rss+xml" title="Gallery RSS" href="http://localhost/PhotoZen/index.php?rss&amp;lang=en_US" />
	*/
		$rssFeedLink = new RssFeedLink();
		
		$rssFeedLink->setData($href);
		$rssFeedLink->setRssTitle($title);

		$this->addDisplayObject($rssFeedLink);

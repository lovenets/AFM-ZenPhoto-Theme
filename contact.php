<?php
	...
	$bodyData .= '">' . gettext('Gallery') . '</a><span class="single"> / ' . gettext('Contact us') . '</span></h2></div></div>';
	$indexPage->addBodyData($bodyData);

	$bodyData = '<div id="contactform" class="ledge">';
	if (function_exists('printContactForm'))
	{
		$bodyData .= captureEcho("printContactForm();");
	}
	else
	{
		$bodyData .= "Please enable the contact form plugin under the admin option panel.";
	}
	$bodyData .= '</div><br />';
	$indexPage->addBodyData($bodyData);
